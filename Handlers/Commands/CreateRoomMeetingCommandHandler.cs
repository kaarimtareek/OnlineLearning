			...
			using (var context = new AppDbContext(contextOptions))
			{
				...
				using var transactionScope = await context.Database.BeginTransactionAsync();
				{
					try
					{
						...
						request.Duration = Math.Abs(request.Duration);
						var zoomResult = await zoomService.CreateZoomMeeting(request.ZoomToken, new Models.NetworkModels.CreateZoomMeetingRequest {
						...
						result = await  meetingService.AddMeeting(context, request.UserId, request.RoomId, zoomResult.Data.join_url, request.StartTime, request.EndTime, request.TopicName, request.TopicDescription,(int) zoomResult.Data.id, zoomResult.Data.password, request.StartNow);
						if(!result.IsSuccess)
                        {
							await transactionScope.RollbackAsync();
